...
import { generateAvatarSvg } from "./avatars.js";
...
export async function renderBadgePng(args: {
  ...
  const { name, role = "Guest", accentHex = "#6366f1", eventTitle } = args;
  const firstName = name.split(" ")[0];

  const avatarPng = svgToPngDataUrl(generateAvatarSvg(name), 200);
